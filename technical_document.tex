...
\title{付録. 再生産関係の推定・管理基準値計算・将来予測シミュレーションに関する技術的ノート \\
  \Large (平成31年度研究機関会議版)
}
\author{ABCWG}
\date{2019/06/25}
...
\begin{equation}
  ...
  \label{future_eq2}
\end{equation}
ここでは$t \geq T_3$で，$t=T_3$のとき$N_t^k=N_t^{\mathrm{obs}}$，$F_t^k=F_t^{\mathrm{obs}}$とする．$\kappa$は漁獲なしのときの親魚量(初期資源量)$S\!B_0$の決定論的な値(式\ref{future_eq})と，確率的予測値(式\ref{future_eq2})の平均を一致させるためのバイアス補正項で，対数正規分布を仮定する場合には$\kappa=-0.5\sigma^2$，残差リサンプリングをおこなう場合には$-\frac{\sum_{t=T_1}^{T_2} \exp(\varepsilon_t)}{n}$となる．
...
\begin{figure}[h]
  \includegraphics[bb=0 0 300 150, width=11cm]{fig_HCR.png}
