...
\title{{\large FRA-SA-2022-ABCWG02-04} \\
...
\begin{figure}[b]
%  \includegraphics[width=4cm]{TAC_carry_over.png}
  \includegraphics{TAC_carry_over.eps}    
  \caption{
...
将来予測において、基本的には漁獲管理規則に従うものの、漁獲量の急激な変動する場合にはその変動に上限（$\mathrm{CVlim}_\mathrm{upper}$）や下限（$\mathrm{CVlim}_\mathrm{lower}$）をつける必要がある場合がある\cite{ichinokawa2022}。漁獲量の変動に上限をつける場合には、式\label{ABC_eq1}または\label{ABC_eq2}から計算される$C_{T_3+2}^k$と$C_{T_3+1}^k$年の漁獲量を比較し、
\begin{eqnarray}
  C_{T_3+2}^{'k} =
  \begin{cases}
    C_{T_3+2}^{k} & C_{T_3+2}^{k}  \le C_{T_3+1}^{k} \times \mathrm{CVlim}_\mathrm{upper} \\
    C_{T_3+1}^k \times \mathrm{CVlim}_\mathrm{upper} & C_{T_3+2}^k  > C_{T_3+1}^k \times \mathrm{CVlim}_\mathrm{upper} \\
  \end{cases}
\end{eqnarray}
とする。下限をつける場合には
\begin{eqnarray}
  ...
  \end{cases}
\end{eqnarray}
とする。下限と上限をつける場合には上式の両方を適用する。将来予測では$T_3+2$年の漁獲量が$C_{T_3+2}^{'k}$となるように$xF_{s,t}^k$を探索的に求め、これを$T_3+2$年の漁獲係数として個体群動態式の中で用いる。
...
\bibitem{rho_simulation} FRA-SA2020-BRP01-06. 市野川桃子, 西嶋翔太, 岡村 寛 (2020) シミュレーションを用いた自己相関係数の同時推定手法の推定バイアス評価（非公開資料）
\bibitem{okamura2020} Okamura H, Ichinokawa M, Hilborn R (2020) Evaluating a harvest control rule to improve the sustainability of Japanese fisheries. bioRxiv. DOI: 10.1101/2020.07.16.207282
\bibitem{ichinokawa2022} 市野川 桃子・西嶋 翔太・ 向 草世香・ 黒田 啓行・ 大下 誠二 (2022) 改正漁業法下での様々な代替漁獲管理規則の検討：マイワシ2系群を例に. 日本水産学会誌. 88: 239-255.
\bibitem{mse_simulation} FRA-SA2020-BRP01-07. 市野川桃子 (2020) 簡易的MSEを用いた複数の管理基準値の頑健性の比較・HCRの検討（非公開資料）
\bibitem{suketou} FRA-SA2020-BRP03-4 (2020) スケトウダラ太平洋系群における資源管理方針に関する検討会（第１回）からの依頼への対応.\url{http://www.fra.affrc.go.jp/shigen_hyoka/SCmeeting/2019-1/HW_suketou-pa_20201211.pdf}
