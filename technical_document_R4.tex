...
\title{{\normalsize FRA-SA-2022-ABCWG02-02} \\
...
結果としてHSの折れ点($b$)が過去最小親魚量または過去最大親魚量となった場合，$b$は仮定値をあてていることになるが，$b$を過去最小親魚量から過去最大親魚量の範囲で探索した結果としての推定結果であると考え，とみなし，AICなどの情報量基準はどのような場合でも$a$, $b$の2パラメータが推定されているものとして計算した．
...
\begin{equation}
  N_{s} = \begin{cases}
    1 &     s = S_\mathrm{min} \\    
    N_{s-1}  \exp(-M_{s-1}- F_{s} )  &    S_\mathrm{min} < s < \infty
  \end{cases}
  \label{spr}
\end{equation}
